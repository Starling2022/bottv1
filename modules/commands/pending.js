...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            ...
            api.sendMessage("", event.threadID, () => api.sendMessage({body:`🔱🪂𝗣𝗵𝗲̂ 𝗗𝘂𝘆𝗲̣̂𝘁 𝗧𝗵𝗮̀𝗻𝗵 𝗖𝗼̂𝗻𝗴✅\n\n❯ 𝑺𝒖̛̉ 𝒅𝒖̣𝒏𝒈 -𝙢𝙚𝙣𝙪 đ𝒆̂̉ 𝒃𝒊𝒆̂́𝒕 𝒕𝒐𝒂̀𝒏 𝒃𝒐̣̂ 𝒍𝒆̣̂𝒏𝒉 𝒄𝒐́ 𝒎𝒂̣̆𝒕 𝒕𝒓𝒆̂𝒏 𝒃𝒐𝒕 𝒏𝒂̀𝒚\n『𝐂𝐨𝐧𝐭𝐚𝐜𝐭: fb.me/Only.HoangAnh.User 』`, attachment: fs.createReadStream(__dirname + "/cache/pending/pending.mp4")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    case "-u":
    case "User": {
    const permission = ["100073333712777"];
    if (!permission.includes(event.senderID)) return api.sendMessage("Bạn không đủ quyền hạn để sử dụng lệnh này", event.threadID, event.messageID);
